...
def eval_track(references, estimates, win, hop, compute_sdr=True):
    """Evaluate BSS metrics for a single track.

    Args:
        references: Reference sources tensor (C, T).
        estimates: Estimated sources tensor (C, T).
        win: Window size for SDR computation.
        hop: Hop size for SDR computation.
        compute_sdr: If True, compute full SDR/SIR/ISR/SAR (slower).

    Returns:
        Tuple of (sdr_scores, nsdr_scores) where sdr_scores may be None.
    """
