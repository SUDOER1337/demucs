...
class Solver(object):
    def __init__(self, loaders, model, optimizer, args):
        ...
        augments = [
            augment.Shift(
                shift=int(args.dset.samplerate * args.dset.shift),
    ...
    def _reset(self):
        ...
        if self.checkpoint_file.exists():
            ...
            model = pretrained.get_model(
                name=self.args.continue_pretrained, repo=self.args.pretrained_repo
            )
    ...
    def train(self):
        ...
        for epoch, metrics in enumerate(self.history):
            ...
            if "test" in metrics:
                ...
                if formatted:
                    logger.info(
                        bold(
                            f"Test Summary | Epoch {epoch + 1} | {_summary(formatted)}"
                        )
                    )
        ...
        for epoch in range(len(self.history), self.args.epochs):
            ...
            if should_eval or is_last:
                ...
                formatted = self._format_test(metrics["test"])
                logger.info(
                    bold(f"Test Summary | Epoch {epoch + 1} | {_summary(formatted)}")
                )
    ...
    def _run_one_epoch(self, epoch, train=True):
        ...
        logprog = LogProgress(
            ...
            data_loader,
            total=total,
            updates=self.args.misc.num_prints,
            name=name,
        )
        averager = EMA()
        ...
        for idx, sources in enumerate(logprog):
            ...
            if not train and self.args.valid_apply:
                estimate = apply_model(
                    self.model, mix, split=self.args.test.split, overlap=0
                )
            else:
            ...
            if train:
                ...
                if args.optim.clip_grad:
                    torch.nn.utils.clip_grad_norm_(
                        self.model.parameters(), args.optim.clip_grad
                    )
